...
### 5.1 dv.current()🍇查询当前页
查询的是，查询语句所在的页面内容。
### 5.2 dv.pages(source)🍈查询指定页
::: tip 查询来源
...
### 7.2 dv.taskList(tasks, groupByFile)任务列表视图
...
```
### 7.3 dv.table(headers, elements)表格视图
...
```
## 8 Markdown Dataviews视图markdown
...
dv.paragraph(table);
```
### 8.1 dv.markdownTable(headers, values)md表格
等效于 `dv.table()` ，它呈现具有给定标题列表和二维元素数组的表，但返回普通Markdown。

### 8.2 dv.markdownList(values)md列表
...
### 8.3 dv.markdownTaskList(tasks)md任务列表
...
```
### 9.3 dv.fileLink(path, [embed?], [display-name])
...
```
### 9.4 dv.sectionLink(path, section, [embed?], [display?])
...
```
### 9.5 dv.blockLink(path, blockId, [embed?], [display?])
...
### 9.11 dv.parse(value)
## 10 File I/O
### 10.1 ⌛ dv.io.csv(path, [origin-file])
...
```
## 11 Query Evaluation
### 11.1 ⌛ dv.query(source, [file, settings])
...
```
### 11.6 dv.evaluate(expression, [context])
